...
module.exports = (client) => {
    ...
    fs.readdirSync(prefixPath).forEach(dir => {
        const commands = fs.readdirSync(`${prefixPath}/${dir}/`).filter(file => file.endsWith('.js'));
    ...
    fs.readdirSync(commandsPath).forEach(dir => {
        ...
        for (const file of commands) {
            ...
            if ('data' in command && 'run' in command) {
                ...
                console.log(`[ WARNING ] : The command at './commands/${dir}/${file}' is missing a required 'data' or 'run' property.`);
            }
        } 
    });
    ...
    for (const file of buttonFiles) {
        ...
        if ('id' in command && 'run' in command) {
            ...
            console.log(`[ WARNING ] : The command at ${filePath} is missing a required 'id' or 'run' property.`);
        }
    }
    ...
    const modalsFiles = fs.readdirSync(modalPath).filter(file => file.endsWith('.js'));
    ...
    fs.readdirSync(eventsPath).forEach(dir => {
        const events = fs.readdirSync(`${eventsPath}/${dir}/`).filter(file => file.endsWith('.js'));
