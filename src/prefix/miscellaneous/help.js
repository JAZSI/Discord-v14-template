...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if (!command) {
            let prefixCategories = [];
            fs.readdirSync(path.join(__dirname, '../../prefix/')).forEach(dir => {
                if (client.config.ignoredCategories.includes(dir)) return;
                if (!prefixCategories.includes(dir)) prefixCategories.push(dir);
            });
            ...
            for (let category of prefixCategories) {
                const dirEmojis = `${client.config.category[category]} ${category.toUpperCase()}`;
                embed1.addFields({ name: dirEmojis, value: '>>> ' + client.prefix.filter(c => c.category === category).map(c => `\`${c.name}\``).join(', '), inline: true });
            }

            let slashCategories = [];
            fs.readdirSync(path.join(__dirname, '../../commands/')).forEach(dir => {
                if (client.config.ignoredCategories.includes(dir)) return;
                if (!slashCategories.includes(dir)) slashCategories.push(dir);
            });
            ...
            const embed2 = new EmbedBuilder()
                ...
                .setTimestamp()
                .setFooter({text: `${client.user.username}`, iconURL: client.user.displayAvatarURL()});
            for (let category of slashCategories) {
                const dirEmojis = `${client.config.category[category]} ${category.toUpperCase()}`;
                embed2.addFields({ name: dirEmojis, value: '>>> ' + client.commands.filter(c => c.category === category).map(c => `\`${c.data.name}\``).join(', '), inline: true });
            }
        ...
        const cmd = client.prefix.get(command) || client.prefix.find(cmd => cmd.aliases && cmd.aliases.includes(command));
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields(
                ...
                { name: 'Voice Channel', value: cmd.voiceChannel ? 'Yes' : 'No', inline: true },
                { name: 'NSFW', value: cmd.nsfw ? 'Yes' : 'No', inline: true },
                { name: 'Disabled', value: cmd.disabled ? 'Yes' : 'No', inline: true },
                { name: 'Owner Only', value: cmd.ownerOnly ? 'Yes' : 'No', inline: true },
                { name: 'Guild Owner Only', value: cmd.guildOwnerOnly ? 'Yes' : 'No', inline: true },
